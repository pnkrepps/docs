...
Column-level conflict detection uses the `column_timestamps` type. This type requires the user to have at least the [bdr_application](../security/pgd-predefined-roles/#bdr_application) role assigned to them.
!!!
